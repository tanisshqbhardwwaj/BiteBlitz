...
if (process.env.STRIPE_SECRET_KEY) {
  ...
  console.warn("STRIPE_SECRET_KEY is not defined. Stripe-related features will fail.");
}

// Place Order for frontend
const placeOrder = async (req, res) => {
  let frontend_url = process.env.FRONTEND_URL || "https://bite-blitz-nine.vercel.app";
  ...
  try {
    ...
    const line_items = req.body.items.map((item) => ({
      price_data: {
        ...
        // using native INR amount (assuming price is in INR)
        unit_amount: item.price * 100,
      },
      quantity: item.quantity,
    }));

    // Pushing the delivery charges (50 INR) in the Line Items
    line_items.push({
      price_data: {
        currency: "inr",
        product_data: {
